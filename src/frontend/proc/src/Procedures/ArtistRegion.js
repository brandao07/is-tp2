...
import {LOAD_BY_REGION_ARTIST} from "../GraphQL/Queries";
...
function ArtistRegion() {
    ...
    const [getTracks] = useLazyQuery(LOAD_BY_REGION_ARTIST);

    const handleClick = () => {
        getTracks({variables: {regionName: searchRegion, artistName: searchArtist}}).then((response) => {
            setDataGraphQL(response.data.regionArtistTracks.filter((item, index) => Math.floor(index / PAGE_SIZE) === (page - 1)));
        })
        axios.get(`${process.env.REACT_APP_API_PROC_URL}/api/regions/artists?region_name=${searchRegion}&artist_name=${searchArtist}`).then((response) => {

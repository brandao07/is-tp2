...
function Regions() {
    ...
    const [maxDataSize, setMaxDataSize] = useState(regions.length);

    useEffect(() => {
        setData(null);
        setTimeout(() => {
            console.log(`fetching from ${process.env.REACT_APP_API_ENTITIES_URL}`)
            setData(regions.filter((item, index) => Math.floor(index / PAGE_SIZE) === (page - 1)));
        }, 500);
    }, [page])
                ...
                <Table sx={{minWidth: 650}} aria-label="simple table">
                    <TableHead>
                        <TableRow>
                            ...
                            <TableCell align="center">Geometry</TableCell>
                        </TableRow>
                    </TableHead>
                    <TableBody>
                        {
